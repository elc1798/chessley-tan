...
from game_reader import board2numpyarray
import heapq
import time
import re
import string
...
def get_model_from_pickle(fn):
    ...
    weight_set, bias_set = trainer.get_parameters(weights=weights, biases=biases)
    in_l, out_l = trainer.get_model(weight_set, bias_set)
    
    predict = tfunc.function(
        inputs=[in_l],
        outputs=[out_l])
